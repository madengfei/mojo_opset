...
@libentry()
@triton.jit
def _top_p_sample_kernel_small(
    sorted_logits_ptr,
    sorted_indices_ptr,
    rand_data_ptr,
    output_ptr,
    ...
    all_topk_indices = tl.load(row_indices_ptr + offsets * stride_indices_k, mask=mask_k, other=0)
...
@libentry()
@triton.jit
def _top_p_sample_kernel_large(
    sorted_logits_ptr,
    sorted_indices_ptr,
    rand_data_ptr,
    output_scores_ptr,
    ...
    row_scores_ptr = output_scores_ptr + pid * stride_out0_b
    row_probs_ptr = output_probs_ptr + pid * stride_out1_b
    noise = tl.load(row_rand_ptr + offsets * stride_rand_k, mask=mask_k, other=1.0)
    eps = 1e-9
    scores = f_probs / (noise + eps)
    tl.store(row_scores_ptr + offsets * stride_out0_k, scores, mask=mask_k)
    tl.store(row_probs_ptr + offsets * stride_out1_k, f_probs, mask=mask_k)
...
def top_p_sampling_impl(
    ...
    if probs_bytes_count <= 20000:
        ...
        grid = (batch_size,)
        _top_p_sample_kernel_small[grid](
            sorted_logits,
            sorted_topk_indices,
            rand_data,
            output_data,
        ...
        grid = (batch_size,)
        _top_p_sample_kernel_large[grid](
            sorted_logits,
            sorted_topk_indices,
            rand_data,
            output_scores,
