...
def top_p_sampling_impl(
    ...
    """Nucleus sampling aligned with ``MojoTopPSampling.forward`` (torch reference).

    The previous Triton paths used inverse-CDF or ``argmax(p / noise)``, which do not
    match ``torch.multinomial`` under a fixed seed (accuracy tests compare against torch).
    """
    logits = logits.to(torch.float32)
